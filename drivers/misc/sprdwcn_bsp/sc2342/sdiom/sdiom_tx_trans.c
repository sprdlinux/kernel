...
static void sdiom_print_buf(void *buf, unsigned int len)
{
	unsigned int i = 0;
	unsigned char *p_byte = NULL;

	p_byte = (unsigned char *)buf;

	for (i = 0; i < len; i++)
		sdiom_print("<0x%x>[0x%x]\n", i, p_byte[i]);
}
#endif
...
static int sdio_send(void *buf, unsigned int len)
{
	...
	sdiom_print
	    ("sdiom:cmd53 sending addr[%p] len_ori[%d] len_align[%d]\n",
	     buf, len, SDIOM_ALIGN_512BYTE(len));

#ifdef TX_PRINT_BUF_SUPPORT
	sdiom_print_buf(buf, len);
#endif
...
void sdiom_tx_trans_task(void)
{
	...
	unsigned int tx_success_bytes = 0;
