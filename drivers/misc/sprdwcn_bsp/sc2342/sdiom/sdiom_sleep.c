...
static struct wake_lock sdiom_dedicated0_wakelock;
...
void sdiom_wakelock_init(void)
{
	...
	wake_lock_init(&sdiom_dedicated0_wakelock, WAKE_LOCK_SUSPEND,
			"sdiom_dedicated0_wakelock");
...
{
	sdiom_print("sdiom, %s enter\n", __func__);
	wake_lock(&sdiom_dedicated0_wakelock);
}

void sdiom_unlock_de0_wakelock(void)
{
	sdiom_print("sdiom, %s enter\n", __func__);
	wake_unlock(&sdiom_dedicated0_wakelock);
}
...
void sdiom_resume_check(void)
{
	...
	while (!atomic_read(&sdiom_resume_flag)) {
		if (cnt == 0) {
			sdiom_info("sdiom, wait sdio resume %s\n", __func__);
			dump_stack();
		}
...
{
	while (!atomic_read(&sdiom_resume_flag)) {
		sdiom_print("sdiom, sleep 5ms wait for sdio resume\n");
		usleep_range(4000, 6000);
	}
}
