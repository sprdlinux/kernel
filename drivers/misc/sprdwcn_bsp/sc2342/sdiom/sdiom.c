...
static unsigned int sdiom_dedicated0_gpio;
static unsigned int dedicated0_irq_num;
...
static irqreturn_t sdiom_dedicated0_irq_handler(int irq, void *para)
{
	sdiom_print("sdiom, %s enter\n", __func__);

	sdiom_lock_de0_wakelock();
	disable_irq_nosync(irq);
...
void sdiom_enable_dedicated0_irq(void)
{
	if (dedicated0_irq_num == 0)
		return;

	irq_set_irq_type(dedicated0_irq_num, IRQF_TRIGGER_HIGH);
	enable_irq(irq_num);
}
...
int sdiom_enable_slave_int(void)
{
	...
	sdiom_print("sdiom, default dedicated int enable: 0x%x\n", reg_val);
	sdio_writeb(sdio_func[FUNC_0],
		    reg_val | VAL_DEINT_ENABLE, SDIOM_FBR_DEINT_EN, &err);
	reg_val = sdio_readb(sdio_func[FUNC_0], SDIOM_FBR_DEINT_EN, &err);

	sdiom_print("sdiom, write after dedicated int enable: 0x%x\n", reg_val);
...
static int sdiom_interrupt_init(void)
{
	...
	sdiom_print("sdiom, %s enter, wakelock init\n", __func__);

	sdiom_print("sdiom, sdiom_gpio_num: %d, dedicated0 gpio: %d\n",
		    sdiom_gpio_num, sdiom_dedicated0_gpio);
	...
	/* dedicated0, have free link interrupt */
	ret = gpio_request(sdiom_dedicated0_gpio, "marlin2_dedicated0_irq");
	if (ret < 0) {
		sdiom_err("req gpio dedicated0 irq = %d fail!!!",
			  sdiom_dedicated0_gpio);
		return ret;
	}

	ret = gpio_direction_input(sdiom_dedicated0_gpio);
	if (ret < 0) {
		sdiom_err("gpio:%d input set fail!!!", sdiom_dedicated0_gpio);
		return ret;
	}

	dedicated0_irq_num = gpio_to_irq(sdiom_dedicated0_gpio);

	ret =
	    request_irq(dedicated0_irq_num, sdiom_dedicated0_irq_handler,
			IRQF_TRIGGER_HIGH | IRQF_NO_SUSPEND,
			"sdiom_dedicated0_irq", NULL);
	if (ret != 0) {
		sdiom_err("request irq err!!!gpio is %d!!!",
			  dedicated0_irq_num);
		return ret;
	}
...
void sdiom_abort(void)
{
	int err;

	sdiom_print("sdiom:sdiom_abort\n");
...
/* Get Success Transfer Byte Before Abort */
unsigned int sdiom_stbba(void)
{
	...
	success_byte = reg_stbba_0 | (reg_stbba_1 << 8) |
	    (reg_stbba_2 << 16) | (reg_stbba_3 << 24);

	sdiom_print("sdiom:stbba:[%d]\n", success_byte);
...
static void sdiom_dump_aon_reg(void)
{
	...
	for (i = 0; i <= DUMP_MIN_REG; i++) {
		...
		sdiom_info("sdiom pmu sdio status:[0x%x]:0x%x\n",
	...
	for (i = 0; i < DUMP_REG_4; i++) {
		...
		sdiom_info("sdiom hready status:[0x%x]:0x%x\n",
	...
	for (i = 0; i < DUMP_REG_4; i++) {
		...
		sdiom_info("sdiom master:%d address:[0x%x]:0x%x\n",
			    ...
			    void *buf, unsigned int len)
{
	...
	sdiom_op_leave();
	if (ret != 0) {
		sdiom_err("sdio dt write fail ret:%d\n", ret);
		sdiom_dump_aon_reg();
		sdiom_abort();
	}
			       ...
			       void *buf, unsigned int len)
{
	...
	sdiom_op_leave();
	if (ret != 0) {
		sdiom_err("sdio dt write bt fail ret:%d\n", ret);
		sdiom_dump_aon_reg();
		sdiom_abort();
	}
			   ...
			   unsigned int len)
{
	...
	sdiom_op_leave();
	if (ret != 0) {
		sdiom_err("sdio dt read fail ret:%d\n", ret);
		sdiom_dump_aon_reg();
		sdiom_abort();
	}
...
static int get_sdio_dev_func(struct sdio_func *func)
{
	if (func->num >= SDIOM_MAX_FUNCS) {
		sdiom_err("sdiom:func num err!!! func num is %d!!!", func->num);
		return -1;
	}
	sdiom_print("sdiom:func num is %d!!!", func->num);
...
static int sdiom_probe(struct sdio_func *func, const struct sdio_device_id *id)
{
	...
	sdiom_info("sdiom: %s Enter\n", __func__);
	sdiom_info("sdiom:func_class: func->class=%x\n", func->class);
	sdiom_info("sdiom:vendor: 0x%04x\n", func->vendor);
	sdiom_info("sdiom:device: 0x%04x\n", func->device);
	sdiom_info("sdiom:func_num: 0x%04x\n", func->num);

	ret = get_sdio_dev_func(func);
	if (ret < 0) {
		sdiom_err("sdiom:get func err!!!\n");
		return ret;
	}
	sdiom_info("sdiom:get func ok!!!\n");
	...
	if (s_rescan_cb != NULL)
		...
		sdiom_err("sdiom:rescan callback not exist\n");
...
static struct mmc_host *sdio_dev_get_host(struct device_node *np_node)
{
	...
	if (pdev == NULL) {
		sdiom_err("sdio dev get platform device failed!!!");
		return NULL;
	}
...
int sdiom_driver_register(void)
{
	...
	if (ret != 0) {
		sdiom_err("sdio register drv err!!!ret is %d!!!", ret);
		return -1;
	}
	sdiom_info("sdiom: register drv succ!!!");
...
int sdiom_init(void)
{
	...
	if (sdiom_init_indicator == true) {
		sdiom_info("sdiom:sdiom already initialized!\n");
		return 0;
	}
	carddetect_indicator = false;
	...
	sdiom_dedicated0_gpio = of_get_named_gpio(np,
	...
	if (dedicated0_irq_num != 0)
		disable_irq(dedicated0_irq_num);
...
{
	sdiom_info("sdiom:sdiom_sdio_rescan\n");

	if (sdio_dev_host == NULL) {
		sdiom_err("sdiom:sdio_sdio_rescan get host failed!");
		return -1;
	}
	mmc_detect_change(sdio_dev_host, 0);
...
{
	sdiom_info("sdiom:carddump flag set[%d]\n", flag);
	if (flag == true) {
		disable_irq(irq_num);
		sdiom_info("sdiom:disable rx int for dump\n");
	}
	card_dump_flag = flag;
}
EXPORT_SYMBOL_GPL(sdiom_set_carddump_status);

unsigned int sdiom_get_carddump_status(unsigned int flag)
{
	/* If assert or dump memory happened, print needed log;
	 * If parse rx data, not print log.
	 */
	if (flag == true)
		sdiom_info("sdiom:carddump flag get[%d]\n", card_dump_flag);
