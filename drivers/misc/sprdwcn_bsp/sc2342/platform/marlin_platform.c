...
void marlin_get_wcn_chipid(void)
{
	...
	ret = sdiom_dt_read(CHIPID_REG, &chip_id, 4);
	if (ret < 0) {
...
static int marlin_download_firmware(void)
{
	...
	while (len < img_size) {
		...
		err = sdiom_dt_write(CP_START_ADDR + len,
...
static int download_firmware(void)
{
	...
	if (err < 0) {
		pr_err("marlin2:%s no find wcnmodem.bin errno:(%d)(ignore!!)\n",
			__func__, err);
	...
	for (i = 0; i < count; i++) {
		...
		err = sdiom_dt_write(CP_START_ADDR + len, buf,
				trans_size);
		if (err < 0) {
			pr_err("marlin %s error:%d\n", __func__, err);
...
void marlin_hold_cpu(void)
{
	...
	ret = sdiom_dt_read(CP_RESET_REG, &reg_val, 4);
	if (ret < 0) {
	...
	reg_val |= 1;
	ret = sdiom_dt_write(CP_RESET_REG, &reg_val, 4);
	if (ret < 0) {
...
void marlin_read_cali_data(void)
{
	...
	if (!wifi_data.cali_data.cali_config.is_calibrated) {
		...
		err = sdiom_dt_read(CALI_OFSET_REG,
	...
	if ((marlin2_clk_wait_reg > 0) && (clk_wait_val > 0)) {
		sdiom_dt_read(marlin2_clk_wait_reg, &cp_clk_wait_val, 4);
		...
		pr_info("marlin2 cp_clk_wait_val is modifyed 0x%x\n",
					cp_clk_wait_val);
		err = sdiom_dt_write(marlin2_clk_wait_reg, &cp_clk_wait_val, 4);
		if (err < 0)
			pr_err("marlin2 write 26M error:%d\n", err);
	}

	/* write this flag to notify cp，AP has read calibration data */
	reg_val = 0xbbbbbbbb;
	err = sdiom_dt_write(CALI_REG, &reg_val, 4);
	if (err < 0) {
...
static int marlin_write_cali_data(void)
{
	...
	ret = sdiom_dt_read(WIFI_REG, &reg_val, 4);
	if (ret < 0) {
		pr_err("marlin read ahb0 fail:%d\n", ret);
		return ret;
	}
	reg_val |= (0x1<<5);
	ret = sdiom_dt_write(WIFI_REG, &reg_val, 4);
	if (ret < 0) {
	...
	ret = sdiom_dt_write(CALI_OFSET_REG, &wifi_data,
	...
	reg_val = 0xaaaaaaaa;
	ret = sdiom_dt_write(CALI_REG, &reg_val, 4);
	if (ret < 0) {
...
static int marlin_start_run(void)
{
	...
	/*set sdio higher priority to visit iram*/
	ret = sdiom_dt_read(CP_SDIO_PRIORITY_ADDR, &reg_val, 4);
	if (ret < 0) {
	...
	ret = sdiom_dt_write(CP_SDIO_PRIORITY_ADDR, &reg_val, 4);
	if (ret < 0) {
...
int marlin_set_wakeup(enum marlin_sub_sys subsys)
{
	...
	while ((ret == NEED_RETRY_WAKE) && (retry_cnt < 5)) {
		msleep(50);
		ret = slp_mgr_wakeup(subsys);
		retry_cnt++;
	}
...
int start_marlin(enum marlin_sub_sys subsys)
{
	if (sdiom_get_carddump_status(true) != 0)
		return 0;
...
int stop_marlin(enum marlin_sub_sys subsys)
{
	if (sdiom_get_carddump_status(true) != 0)
		return 0;
...
static int marlin_probe(struct platform_device *pdev)
{
	...
	init_completion(&marlin_dev->carddetect_done);
	sdiom_register_rescan_cb(marlin_scan_finish);
	sdiom_init();
