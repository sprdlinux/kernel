...
static atomic_t  cp2_state, wake_flag, exit_flag, wake_complete_flag;
static struct mutex    drv_slp_lock;
...
int slp_mgr_wakeup(enum marlin_sub_sys subsys)
{
	...
	mutex_lock(&drv_slp_lock);
	if (atomic_read(&wake_complete_flag) != 1) {
		SLP_MGR_INFO("last wake not complete, need retry\n");
		mutex_unlock(&drv_slp_lock);
		return NEED_RETRY_WAKE;
	}
	...
	if (atomic_read(&cp2_state) == STAY_SLPING) {
		...
		atomic_set(&(wake_complete_flag), 0);
		ret = sdiom_aon_writeb(REG_CP_SLP_CTL, 0);
		...
		if (ret == 0) {
			...
			atomic_set(&(wake_complete_flag), 1);
			return -ETIMEDOUT;
		}
...
static int sdio_ap_int_clr0(void)
{
	unsigned char reg_int_clr0 = 0;

	sdiom_aon_readb(REG_AP_INT_CLR0, &reg_int_clr0);
	reg_int_clr0 |= BIT_AP_INT_CLR0;
	sdiom_aon_writeb(REG_AP_INT_CLR0, reg_int_clr0);

	return 0;
}

static int sdio_ap_int_ack_clr0(void)
{
	unsigned char reg_int_clr0 = 0;

	sdiom_aon_readb(REG_AP_INT_CLR0, &reg_int_clr0);
	reg_int_clr0 |= BIT_CP_ACK_AP;
	sdiom_aon_writeb(REG_AP_INT_CLR0, reg_int_clr0);

	return 0;
}

static int sdio_ap_int_cali_clr0(void)
{
	unsigned char reg_int_clr0 = 0;

	sdiom_aon_readb(REG_AP_INT_CLR0, &reg_int_clr0);
	reg_int_clr0 |= BIT_CP_CALI_NOTIFY_AP;
	sdiom_aon_writeb(REG_AP_INT_CLR0, reg_int_clr0);
...
static unsigned int ack_cnt, first_req_slp;
...
int slp_isr_handle_thread(void *data)
{
	...
	while (!kthread_should_stop()) {
		...
		/* SLP_MGR_INFO("slp_mgr_isr,INT_STS0-0x%x\n", */
			/* reg_ap_pub_int_sts0); */

		if (reg_ap_pub_int_sts0 & BIT_CP_CALI_NOTIFY_AP) {
			sdio_ap_int_cali_clr0();
		...
		if (flag_download) {
			...
			if (reg_ap_pub_int_sts0 & BIT_CP_ACK_AP) {
				sdio_ap_int_ack_clr0();
				if (atomic_read(&wake_flag) == 1) {
					complete(&wakeup_ack);
...
static irqreturn_t slp_mgr_req_sleep_isr(int irq, void *para)
{
	...
	if (atomic_read(&wake_flag) == 1) {
		...
		atomic_set(&(wake_flag), 0);
		ack_cnt++;
		SLP_MGR_INFO("irq_ack_cnt-%d!!", ack_cnt);
	}
